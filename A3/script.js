...
let walls = [];
let circle;
let timer = 30 * 1000; // 30 seconds in milliseconds
let startTime;
let playerImage; // Variable to hold the player image
let playerImageRadio; // Variable to hold the radio buttons

function preload() {
    
    playerImage = loadImage('assets/among us red.png');
}
...
cnv.parent('column-two')
    ...
    playerImageRadio.option( 'assets/among us red.png', 'Player 1');
    playerImageRadio.option( 'assets/blue amonng us.png', 'Player 2');
    playerImageRadio.option( 'assets/white among us.png','Player 3');
    playerImageRadio.position(10, 10);
    playerImageRadio.style('width', '200px');
...
function draw() {
    ...
    if (remainingTime <= 0) {
        ...
        text("Time Remaining: " + seconds + " seconds", 20, 20);
    }
}
